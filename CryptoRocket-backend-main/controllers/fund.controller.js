...
exports.Fundadd = async (req, res) => {
  try {
    ...
    if (!user.active.isActive && amountNumber >= 100) {
      user.active.isActive = true;
      user.active.activeDate = new Date();
    }
    await newTransaction.save();
    if (user.sponsor) {
      // ✅ Distribute referral bonus using new system
      const { distributeReferralBonus } = require('../utils/referralBonus.calculation');
      await distributeReferralBonus({ userId: user._id, amount: Number(amount) });
    }
    await user.save();
    res.status(200).json({
      success: true,
...
exports.Fundget = async (req, res) => {
  try {
    const transactions = await TransactionModel.find({
      ...
      mainAddress: "Admin Deposit",
      status: "Completed",
    }).populate({ path: "user", select: "username account" }).sort({ createdAt: -1 });
    
    res.status(200).json({
      success: true,
      message: "Add Fund History fetched successfully",
      data: transactions,
    });
  } catch (error) {
    console.log(error);
