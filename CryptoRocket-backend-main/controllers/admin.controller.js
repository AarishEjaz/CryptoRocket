...
exports.AdminLogin = async (req, res) => {
    ...
    try {
        ...
        admin.token = token;
        await admin.save();
        res.cookie('ZTD_admin', token, { httpOnly: true, maxAge: 30 * 24 * 60 * 60 * 1000 });
        return res.status(200).json({ success: true, message: "Logged in successfully.", data: admin, role: admin.role, token });
    } catch (error) {
        console.log(error)
        return res.status(500).json({ success: false, message: error.message });
    }
...
exports.AdminLogout = async (req, res) => {
    try {
        ...
        if (admin) {
            admin.token = null;
            admin.tokenBlock.push(req.cookies.ZTD_admin);
            await admin.save();
        }
...
exports.getTradingProfitIncomes = async (req, res) => {
    try {
        const history = await CommissionIncome.find({ type: "Trading Profit Income" }).sort({createdAt: -1}).populate([{ path: "user", select: 'id username -_id' }, { path: "package", select: "title -_id" }])
...
exports.getLevelIncomes = async (req, res) => {
    try {
        const history = await CommissionIncome.find({ type: "Level ROI Dividend" }).populate({ path: "user fromUser", select: 'id username -_id' })
        const totalIncome = history.reduce((sum, investment) => sum + investment.income, 0);
        const startOfToday = new Date();
        startOfToday.setHours(0, 0, 0, 0);
        const todayLevelIncome = history.filter(investment => investment.createdAt >= startOfToday);
        const todayTotal = todayLevelIncome.reduce((sum, investment) => sum + investment.income, 0);
        res.status(200).json({ success: true, message: "Level ROI Dividend Reports", data: { history, totalIncome, todayTotal } })
    } catch (error) {
        res.status(500).json({ success: false, message: error.message })
    }
}
...
exports.getIncomeSummary = async (req, res) => {
    try {
        ...
        const incomeSources = {
            ...
            level: { model: CommissionIncome, field: "income", match: { type: "Level ROI Dividend" } },
...
exports.getAdminStats = async (req, res) => {
    try {
        ...
        // Level Income (Level ROI Dividend)
        const todayLevelAgg = await CommissionIncome.aggregate([
            { $match: { type: "Level ROI Dividend", createdAt: { $gte: startOfDay } } },
            { $group: { _id: null, total: { $sum: "$income" } } }
        ]);
        const totalLevelAgg = await CommissionIncome.aggregate([
            { $match: { type: "Level ROI Dividend" } },
            { $group: { _id: null, total: { $sum: "$income" } } }
        ]);

        // Trading Profit Income
        const todayTradingAgg = await CommissionIncome.aggregate([
            { $match: { type: "Trading Profit Income", createdAt: { $gte: startOfDay } } },
            { $group: { _id: null, total: { $sum: "$income" } } }
        ]);
        const totalTradingAgg = await CommissionIncome.aggregate([
        ...
        const todayDepositAgg = await TransactionModel.aggregate([
            { $match: { type: "Deposit", createdAt: { $gte: startOfDay } } },
            { $group: { _id: null, total: { $sum: "$investment" } } }
        ]);
        const totalDepositAgg = await TransactionModel.aggregate([
            { $match: { type: "Deposit" } },
            { $group: { _id: null, total: { $sum: "$investment" } } }
        ]);
