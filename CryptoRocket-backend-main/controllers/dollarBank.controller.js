...
exports.DollarBankInvestment = async (req, res) => {
  try {
    ...
    const transactionId = generateCustomId({ prefix: "ZTD-TX", min: 10, max: 10 });
...
exports.DollarBankWithdrawal = async (req, res) => {
  try {
    ...
    // ✅ Distribute Single Leg Income from withdrawal (10% of withdrawal amount)
    const { distributeSingleLegFromWithdrawal } = require('./roiAndSingleLeg.controller');
    const singleLegAmount = dollarBankInvestment.totalAmount * 0.10; // 10% of withdrawal
    await distributeSingleLegFromWithdrawal(user._id, singleLegAmount);
