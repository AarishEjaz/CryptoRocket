...
import { getUserFundRejected } from "../../api/admin.api";
...
const UserFundRejected = () => {
  const location = useLocation();
  const data = location.state;
  const [userFundRejected, setUserFundRejected] = useState([]);
  const dispatch = useDispatch();
  const fetchUserFundRejected = async () => {
    try {
      dispatch(setLoading(true));
      const response = await getUserFundRejected();
      setUserFundRejected(response?.data);
    } catch (err) {
      console.log(err);
    } finally {
  ...
  useEffect(() => {
    fetchUserFundRejected();
  }, []);

  const filteredUsers = userFundRejected?.filter((user) => {
    if (data === "active") {
...
export default UserFundRejected;
